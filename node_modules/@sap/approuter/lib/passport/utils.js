...
module.exports = {
  ...
  storeToken: function (req, options, done) {
    req.session.regenerate(function (err) {
      ...
      req.app.tenantInfo = Object.assign({}, req.app.tenantInfo, {
  ...
  callUaa: function (requestOptions, shouldDoRequest, cb) {
    ...
    // Use caller-provided retryCondition if present, otherwise use default
    requestOptionWithRetry.retryCondition = requestOptions.retryCondition || customRetryCondition;
    ...
    request.post(requestOptionWithRetry, function onResponse(err, res, body) {
      if (err || res.statusCode !== 200) {
        let data;
        const responseData = res && res.response && res.response.data;
        if (responseData && typeof responseData === 'object') {
          ...
          try {
            ...
            tracer.info('Response body from %s is not JSON (status %d): %s — %s', requestOptions.url, res.statusCode, responseData, parseErr.message);
          }
        }
        const reason = data ? `${data.error}, error description: ${data.error_description}` : (responseData || res.message || 'no reason provided');
