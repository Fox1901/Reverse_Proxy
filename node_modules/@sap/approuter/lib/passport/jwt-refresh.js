  ...
  refreshToken: function (app, session) {
    ...
    memoryStore.getSessionTimeout(session.id, function (err, sessionTimeout) {
      ...
      memoryStore.update(session.id, function (newSession) {
        ...
        // Also update req.session so the end-of-request set() writes the flag to Redis.
        // memoryStore.update operates on this.sessions (a separate object from req.session),
        // so without this the end-of-request write would overwrite the flag back to false.
        session.jwtRefreshStarted = true;
        session.jwtRefreshStartedAt = newSession.jwtRefreshStartedAt;
...
function doRefresh(app, session) {
  ...
  passportUtils.callUaa(requestOptions, checkTimeout, function (err, uaaResponse) {
    ...
    }, false, true); // flushToRedis=true: bypass in-flight guard so clearing write reaches Redis
