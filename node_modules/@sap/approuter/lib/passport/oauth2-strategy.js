...
OAuth2Strategy.prototype.authenticate = async function (req, options) {
  ...
  const isLoginCallback = loginCallbackProvider.isLoginCallback(req);
  // Only treat query.error as an OAuth error on the actual login callback path — prevents ?error= injection on protected routes.
  if (req.query && req.query.error && isLoginCallback) {
  ...
  let originalUrl;
  if (!req.query || !req.query.code || (req.query.code && !isLoginCallback)) {
    let location;
    try {
