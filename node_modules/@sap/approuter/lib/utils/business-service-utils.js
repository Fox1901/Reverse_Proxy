...
function downloadHTML5Application(req, res) {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    let appKey = dynamicRoutingUtils.getApplicationKey({url: '/' + (parsedPath.split('/')[3] || '')});
    let appHostId = null;
    let html5RepoToken;
    try {
    ...
    if (prop === 'tokenServiceURL') {
      ...
      if (rawUrl) {
        ...
        if (urlObj.hostname && urlObj.protocol) {
          ...
          tracer.error('Invalid tokenServiceURL for destination ' + destination.Name + ': ' + rawUrl);
        }
      }
    }
    if (prop === 'xsappname') {
