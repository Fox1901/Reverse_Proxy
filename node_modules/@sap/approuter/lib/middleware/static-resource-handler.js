...
const contentTypeUtil = require('../utils/content-type');
...
module.exports = function staticResourceHandler(req, res, next) {
  ...
  let fullDirName = path.resolve(routerConfig.workingDir, url.route.localDir);
  const resolvedWorkingDir = path.resolve(routerConfig.workingDir);
  tracer.info('[STATIC] Serving static path:', fullDirName);

  let replacements = url.route.replace;
  if (!replacements) {
    return serveStaticContent(fullDirName, resolvedWorkingDir, req, res, next, url);
  }
  if (_.endsWith(url.pathname, '/')) {
    url.pathname += 'index.html';
  }
  let filePath = path.join(fullDirName, url.pathname);
  ...
  fs.realpath(filePath, function (err, resolvedPath) {
    ...
    if (!resolvedPath.startsWith(resolvedWorkingDir + path.sep) && resolvedPath !== resolvedWorkingDir) {
      return sendError(req, new VError('Path traversal! Requested path: %s', resolvedPath), next, 403);
    }
    let isMatching = replacements.pathSuffixes.some(function (suffix) {
      return _.endsWith(url.pathname, suffix);
    });
    if (isMatching) {
      serveReplacedFile(resolvedPath, req, res, next, replacements.view);
    } else {
      serveStaticContent(fullDirName, resolvedWorkingDir, req, res, next, url);
    }
  });
};

function serveStaticContent(fullDirName, resolvedWorkingDir, req, res, next, url) {
  if (url.pathname === '') {
  ...
  const requestedFilePath = path.join(fullDirName, url.pathname || '');
  if (!requestedFilePath.startsWith(resolvedWorkingDir + path.sep) && requestedFilePath !== resolvedWorkingDir) {
    return sendError(req, new VError('Path traversal! Requested path: %s', requestedFilePath), next, 403);
  }
