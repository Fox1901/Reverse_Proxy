...
const self = module.exports = {
    ...
    self.replaceUserToken(options, function (err, userExchangeToken, token, iasTokenFlow) {
      ...
      destinationUtils.findDestination(destinationName, accessToken, options, function (err, destinationLookUpResult) {
        ...
        try {
          ...
          if (destinationLookUpResult.authTokens && destinationLookUpResult.authTokens[0] && destinationLookUpResult.authTokens[0].hasOwnProperty('error')) {
            return cb(destinationLookUpResult.authTokens[0].error);
          }
          if (destinationLookUpResult.authTokens && destinationLookUpResult.authTokens[0]) {
          ...
          result.expireDate = destinationLookUpResult.authTokens && destinationLookUpResult.authTokens[0] && destinationLookUpResult.authTokens[0].expireDate;
          ...
          result.authToken = destinationLookUpResult.authTokens && destinationLookUpResult.authTokens[0] ? destinationLookUpResult.authTokens[0] : null;
  ...
  getIASTokenByIASDependencyName: async function(options) {
    ...
    if (IASDependencyName && loginIdToken && options.zoneInfo){
      ...
      const credentials = Object.assign({}, options.session.user.token.oauthOptions, {
        tokenEndPoint: options.zoneInfo.tokenEndpoint,
        apptid: options.zoneInfo.apptid
      });
