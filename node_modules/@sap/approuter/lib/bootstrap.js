...
const lazyJwtRefreshMiddleware = require('./middleware/lazy-jwt-refresh-middleware');
...
module.exports = function bootstrap(options) {
  ...
  if (routerConfig.jwtRefresh !== 0 && !cookieUtils.isSessionStickinessEnabled()) {
    // Non-sticky mode: lazy inline refresh BEFORE loginMiddleware so that
    // loginMiddleware sees a fresh valid token (not an expired one that would
    // trigger re-authentication).
    app.use(lazyJwtRefreshMiddleware);
  }
  app.use(loginMiddleware);
  if (routerConfig.jwtRefresh !== 0 && cookieUtils.isSessionStickinessEnabled()) {
    // Sticky mode only: background timer-based refresh (original behaviour)
