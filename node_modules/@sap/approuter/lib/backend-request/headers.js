...
exports.getHeaders = function (req, accessToken, destination) {
  ...
  if (req) {
    ...
    if (route && route.credentials && businessServiceUtils.getGrantType(route.credentials) === businessServiceUtils.MTLS
        && req.session && req.session.user) {
      tenantInfo && tenantInfo.tenantid && !headers['x-tenant-id'] && (headers['x-tenant-id'] = tenantInfo.tenantid);
    }
  }
  tenantInfo && tenantInfo.logInfo && !headers.tenantid && (headers.tenantid = tenantInfo.logInfo);
...
function addOauthHeader(headers, tokens, route, destination, logger) {
  ...
  if ((route && route.destination) || (destination && destination.isLogout)) {
    ...
    } else if (businessServiceUtils.getGrantType(route.credentials) === businessServiceUtils.MTLS) {
      // mTLS: no Authorization header — authentication is via client certificate on the TLS connection
      logger && (logger.info(`mTLS connection for service ${route.service} - skipping Authorization header`));
    } else {
      headers.authorization = 'Bearer ' + tokens.accessToken;
